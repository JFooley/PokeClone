...
def moveDamage(atacker: Pokemon, defender: Pokemon, move: Move, effeChart: dict):
    # Calcular o critico
    ...
    if move.kind == "Physical":
        damage = ((atacker.attack/defender.defense) * move.power * STAB * CRIT * float(effeChart[move.type][defender.type1]) * float(effeChart[move.type][defender.type2] if defender.type2 else 1))

        return int(round(damage))
    
    elif move.kind == "Special":
        damage = ((atacker.spAtk /defender.spDef) * move.power * STAB * CRIT * float(effeChart[move.type][defender.type1]) * float(effeChart[move.type][defender.type2] if defender.type2 else 1)) 
